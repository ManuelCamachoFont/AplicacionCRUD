...
public class BajaActor extends WindowAdapter implements ActionListener
{
	...
	public void datos()
	{
		...
		try
		{
			...
			while (rs.next())
			{
				lista.add(rs.getInt("idActor") + " | " + rs.getString("nombreActor") + " | "
	...
	public void darBaja()
	{
		...
		String sentencia = "DELETE from actores WHERE idActor = " + lista.getSelectedItem().split(" ")[0];
		...
		try
		{
			...
			lblDiaFeedback.setText("Se ha eliminado al actor");
		} catch (ClassNotFoundException cnfe)
		{
			diaFeedback.setBackground(new Color(243, 70, 74));
